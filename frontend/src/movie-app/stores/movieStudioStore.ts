...
/** Output resolution (square) for scene images generated by Qwen Image Edit. */
export type SceneImageResolution = "512" | "1024";

/** Resolve pixel dimensions for a scene image resolution. */
function sceneImageDimensions(
  resolution: SceneImageResolution,
): { width: number; height: number } {
  const n = Number(resolution) || 1024;
...
interface MovieStudioStore {
  ...
  sceneImageResolution: SceneImageResolution;
  ...
  setSceneImageResolution: (v: SceneImageResolution) => void;
...
export const useMovieStudioStore = create<MovieStudioStore>((set, get) => ({
  ...
  setSceneImageSteps: (steps) =>
    set({
      sceneImageSteps: Math.min(20, Math.max(8, Math.round(Number(steps)) || 8)),
    }),

  setSceneImageResolution: (sceneImageResolution) =>
    set({ sceneImageResolution }),
  ...
  render: async (projectId) => {
    ...
    const dims = sceneImageDimensions(get().sceneImageResolution);
